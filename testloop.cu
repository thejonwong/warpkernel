...
int main(int argc, char *argv[]) {
  ...
  // boost::accumulators::accumulator_set<uint, boost::accumulators::stats<boost::accumulators::tag::mean  > cusp_csr;
  // boost::accumulators::accumulator_set<uint, boost::accumulators::stats<boost::accumulators::tag::mean  > cusp_hyb;
  // boost::accumulators::accumulator_set<uint, boost::accumulators::stats<boost::accumulators::tag::mean  > wk1;
  // boost::accumulators::accumulator_set<uint, boost::accumulators::stats<boost::accumulators::tag::mean  > wk1_no;
  // boost::accumulators::accumulator_set<uint, boost::accumulators::stats<boost::accumulators::tag::mean  > wk1_rex;

  for (int t = 0; t < ntests; t++) {

    bool lastiter = (t == ntests-1);
    // cusp multiplication
    {

      static boost::accumulators::accumulator_set<ValueType, boost::accumulators::stats<boost::accumulators::tag::mean>  > statstime;
	...
	std::cout << "cusp gpu time " 
		  << std::scientific << measuredtime << std::endl;
	  ...
	  if (eng.verify(y,ycheck) && lastiter)
	    {
	      std::cout << "warpkernel (" << nblocks << "," << blocksize <<") time = " 
			<< std::scientific << measuretime << std::endl;
	      warpkernel::addData(datafile, "warpkernel", measuretime, kernel1, blocksize);
	    }  else  exit(1);
	...
	// reordered kernel without rowmap
	{
	  ...
	  if (eng.verify_x(y,ycheck) && lastiter) {
	    std::cout << "warpkernel reorder w/o rowmap (" << nblocks << "," << blocksize <<") time = " 
		      << std::scientific << measuretime << std::endl;
	    warpkernel::addData(datafile, "warpkernel_no", measuretime, kernel1, blocksize);
	  } else exit(1);
	...
	// reordered kernel with rowmap
	{
	  ...
	  if (eng.verify(y,ycheck) && lastiter) {
	    std::cout << "warpkernel reorder (" << nblocks << "," << blocksize <<") time = " 
		      << std::scientific << measuretime << std::endl;
	    warpkernel::addData(datafile, "warpkernel_rex", measuretime, kernel1, blocksize);
	  } else exit(1);
	  eng.device_colinds = restore_col;
	...
	for(int warps_per_block = 2; warps_per_block <= 8; warps_per_block ++) {
	  ...
	  // normal kernel
	  {
	    ...
	    if (eng.verify(y,ycheck) && lastiter) {
	      ...
	      warpkernel::addData(datafile, (char *) (kernelname.str()).c_str(), measuretime, 
				  kernel2, blocksize);
	    } else exit(1);
	  }
	...
	ValueType measuretime = mgputime.seconds_elapsed()/ntests;

	std::cout << "mgpu time" << valuesPerThread << "\t" << measuretime << " s "  << " status :" << status << std::endl;
	...
	if (check) {
	  ...
	  warpkernel::addData(datafile, (char *) (kernelname.str()).c_str(), measuretime, -1,-1,-1,-1);
	}
	...
	status2 = cusparseDcsrmv(handle,CUSPARSE_OPERATION_NON_TRANSPOSE, N, N, 1.0,
      ...
      ValueType measuretime = cusparse.seconds_elapsed()/ntests;
